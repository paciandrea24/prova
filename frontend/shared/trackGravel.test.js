...
// nastro si ripiega su se stesso, formando una cuspide e poi un cappio. In
// gioco l'utente lo ha visto come "groviglio di barriere" e poi come "questa
// specie di cuspide", il 2026-08-12, ai campioni 133/338/648/763 di prova.
//
...
function ripiegamenti(pts, distDi) {
    ...
    for (const side of [-1, 1]) {
        for (let i = 0; i < n; i++) {
            ...
            // Avanzamento del punto di barriera da prev a i. La normale di
            // prev è perpendicolare a t per costruzione, quindi entra solo la
            // distanza di i.
...
for (const id of ['prova', 'monte-rosso', 'new-monza', 'baku']) {
    test(`barrierProfile: le vie di fuga non ripiegano il nastro (${id})`, () => {
        const { raw, pts } = pistaVera(id);
        const bar = TrackGravel.barrierProfile(pts, { roadHalf: raw.roadHalfWidth });
        ...
        // prima che questo modulo esistesse. Su baku e new-monza qualche curva
        // è più stretta del muro stesso e si ripiega comunque: è geometria di
        // quei tracciati, non un effetto delle vie di fuga. Quello che le vie
        // di fuga NON possono fare è peggiorare il conto.
        const prima = ripiegamenti(pts, () => storica);
        const adesso = ripiegamenti(pts, (i, side) => TrackGravel.barrierAt(bar, i, side));
