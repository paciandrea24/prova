...
for b in range(4):   # bracci: 0=N, 1=E, 2=S, 3=O (phi = 90*b orario da nord)
    ...
    # EST (b==1) resta sigillato; N/S/O aperti = aggancio ai corridoi verso Jazz
    add("portale" if b == 1 else "portale_aperto", x, z, -phi)
...
gl.add_cyl("base_rotonda", 10.75, 0.05, 0, 0, 0.0, mats['nero'], vertices=48)
for phi in (0, 90, 180, 270):
    gx, gz = rot(0, -(Z_M + Z_END) / 2, phi)
    box = gl.add_box(f"base_braccio_{phi}", 9.6, Z_END - Z_M + 0.6, 0.05, gx, -gz, 0.0, mats['nero'], bevel=0)
